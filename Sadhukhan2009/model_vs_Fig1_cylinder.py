...
# experimental data taken from Figure 1 (cylinder) in Sadhukhan2009
...
Tcyl = np.loadtxt('Fig1_Tcylinder.csv', delimiter=',')
mcyl = np.loadtxt('Fig1_Mcylinder.csv', delimiter=',')
...
py.figure(3)
py.plot(t, hg[:, 0], label='center')
py.plot(t, hg[:, m-1], label='surface')
py.legend(loc='best', numpoints=1)
py.xlabel('Time (s)')
py.ylabel(r'Heat Generation ($W/m^3$)')
py.show()
py.grid()

# heat generation vs temperature
py.figure(4)
py.plot(TT[:, 0], hg[:, 0])
py.plot(TT[:, m-1], hg[:, m-1])
py.grid()
py.show()
