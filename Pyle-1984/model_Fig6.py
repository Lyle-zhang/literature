"""
Compare 1-D transient heat conduction model to Figure 6 in Pyle 1984
"""
...
h = 90          # heat transfer coefficient, W/m^2*K
Ti = 303        # initial particle temp, K
Tinf = 753      # ambient temp, K
...
t1, Temp = np.loadtxt('Fig6temp.csv', delimiter=',', unpack=True)
t2, Mass = np.loadtxt('Fig6conv.csv', delimiter=',', unpack=True)

# r/R axis normalized from 0 to 1
rR = np.arange(0,r+dr,dr)/r

# row id for 2 min (120 s), 3 min (180 s), 4 min (240 s), 6 min (360 s)
id2min = np.where(t==120)[0][0]
id3min = np.where(t==180)[0][0]
id4min = np.where(t==240)[0][0]
id6min = np.where(t==360)[0][0]
...
py.plot(rR, T[id3min], '-r', label='3 min')
py.plot(rR, T[id4min], '-b', label='4 min')
py.plot(rR, T[id6min], '-c', label='6 min')
